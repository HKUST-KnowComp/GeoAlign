...
def reconstruction_worker(adj, lt, distfn, leaves, progress=False, filter=True,
                          hits_conf=[1,3,10], objects=[]):
    ranksum = nranks = ap_scores = iters = mrr = 0
    labels = np.empty(lt.size(0))
    hits_ary = np.full(len(hits_conf), 0)
    for object in tqdm(objects) if progress else objects:
        ...
        sorted_idx_filter = np.array([i for i in sorted_idx if i not in leaves])
        if filter:
            ranks, = np.where(np.in1d(sorted_idx_filter, neighbors))
        else:
        ...
        for i in range(0, N):
            rr += 1 / ranks[i]
        mrr += rr / N
        for i, hi in enumerate(hits_conf):
            hits_ary[i] += int(ranks[0] <= hi)
        ...
        labels[neighbors] = 1
        labels_filter = np.array([labels[i] for i in range(len(labels)) if i not in leaves])
        label_score = -dists.cpu().numpy()
        label_score_filter = np.array([label_score[i] for i in range(len(label_score)) if i not in leaves])
        if filter:
            ap_scores += average_precision_score(labels_filter, label_score_filter)
        else:
            ap_scores += average_precision_score(labels, label_score)
        iters += 1
    return np.array((float(ranksum), nranks, ap_scores, iters, mrr, ) + tuple(hits_ary))


def eval_reconstruction(adj, lt, distfn, leaves, workers=1, progress=False, filter=True, hits_conf=[1,3,10]):
    '''
    ...
    Args:
        ...
        leaves: the set of leaves in taxonomy
    '''
    objects = np.array(list(adj.keys()))
    if workers > 1:
        with ThreadPool(workers) as pool:
            f = partial(reconstruction_worker, adj, lt, distfn, leaves, False, filter, hits_conf)
            results = pool.map(f, np.array_split(objects, workers))
            results = np.array(results).sum(axis=0).astype(float)
    else:
        results = reconstruction_worker(adj, lt, distfn, leaves, progress, filter, hits_conf, objects)
    return float(results[0]) / results[1], float(results[2]) / results[3], float(results[4]) / results[3],  results[5:] / results[3]
